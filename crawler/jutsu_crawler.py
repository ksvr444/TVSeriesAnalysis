...
class BlogSpider(scrapy.Spider):
    ...
    def parse_jutsu(self, response):
        jutsu_name = response.css("span.mw-page-title-main::text")[0].extract()
        ...
        soup.find("aside").decompose() # decompose means removing the aside section

        jutsu_description = soup.text.strip()
        jutsu_description = jutsu_description.split("Trivia")[0].strip()

        return {'jutsu_name': jutsu_name, 'jutsu_type': jutsu_type, 'jutsu_description': jutsu_description}
